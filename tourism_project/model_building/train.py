...
# Optional: MLflow tracking URI (local file-based tracking)
...
def build_model(class_weight=None):
    """
    Build an XGBoost classifier wrapped in a pipeline with StandardScaler.
    """
...
def get_class_weight(y_train):
    """
    Compute a simple class weight for the positive class (1) to handle imbalance.
    """
    ...
    if 0 in value_counts and 1 in value_counts:
        ...
        print("Computed scale_pos_weight for positive class:", class_weight)
        return class_weight
    else:
...
def upload_model_to_hf(model_path: str):
    ...
    # Create or use existing model repo
    try:
